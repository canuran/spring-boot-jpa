...
public class SecurityUserService implements UserDetailsService {
    ...
    public UserDetails loadUserByUsername(String username)
        ...
        // 获取用户信息
        SecurityUser securityUser = userService.getByName(username);
        if (securityUser == null) {
            throw new UsernameNotFoundException("用户名不存在或已删除。");
        }

        // 获取用户角色
        List<RoleAsAuthority> authorities = userService.getUserRoles(securityUser.getId());
        securityUser.setAuthorities(authorities);

        // 获取用户权限
        List<PermissionTree> permissions = userService
                .getUserPermissions(securityUser.getId());
